...
from ..common import gologin_repo_provider
...
@gologin_repo_provider
async def set_apikey(
        message: Message, state: FSMContext,
        gologin_service: GologinApikeysRepository):
    await state.clear()

    if not len(message.text.split(".")) == 3:
        return await message.reply("✅Ввод отменен")

    gologin_service.set(new_apikey=message.text.replace(" ", ""))

    await message.reply(
        text=f"✅Ключ сохранен:\n\n <code>{gologin_service.get_current()}</code>"
    )
